# source: https://github.com/nyurik/py-ascii-graph/blob/fix-python310/ascii_graph/__init__.py
...
class Pyasciigraph:
    ...
    def _gen_graph_string(
        ...
        def _gen_graph_string_part(
            ...
            if all_width == 0:
                ...
                bar_width = int(abs(float(value)) *
                                float(graph_length) / float(all_width))
        ...
        if all_width == 0:
            ...
            neg_width = int(abs(float(min_neg_value)) *
                            float(graph_length) / float(all_width))
        ...
        if isinstance(value, Iterable):
            ...
            totalstring = Pyasciigraph._u(
                ' ') * (neg_width - abs(totalsquares)) + totalstring
            ...
            if value >= 0:
                return Pyasciigraph._u(' ') * neg_width + \
                    partstr + \
                    Pyasciigraph._u(' ') * (start_value_pos -
                                            (neg_width + squares))
            else:
    ...
    def _gen_info_string(self, info, start_info_pos, line_length):
        ...
        """
        number_of_space = (line_length - start_info_pos -
                           self._len_noansi(info))
